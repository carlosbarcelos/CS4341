...
public abstract class StateTree
{
	...
	public int lastColumn; // Where was the last column last placed
	public int lastRow;    // Where was the last row last placed
	public boolean full; // Specifies if the board is full or not
	...
	public StateTree(int r, int c, int w, int t, boolean p1, boolean p2, StateTree p)
	{
		...
		this.full = false; // Assume a board is not empty to begin with
	}
	...
	public boolean validMove(Move move)
	{
		...
		{
			//out.println("That column is full." + move);
			return false;
		}
		if(move.pop)
		{
	...
	// Returns whether or not a board has any more spaces
	...
	// Get the row depenent on the column
	// This function will never take an invalid move
	public int getRow(Move move){
		int row = -1; // If -1 returned, you messed up
		for(row=0; row<rows; row++)
		{
			if(boardMatrix[row][move.column] == 0)
			{
				break;
			}
		}
	...
	public abstract StateTree copy();
}
